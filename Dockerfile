...
RUN chroot /var/shrubbery/sandbox/sandbox-origin python3.12 -m venv venv
RUN chroot /var/shrubbery/sandbox/sandbox-origin . venv/bin/activate && python3.12 /tmp/get-pip.py
RUN chroot /var/shrubbery/sandbox/sandbox-origin . venv/bin/activate && python3.12 -m pip install timeout_decorator
RUN useradd tester
